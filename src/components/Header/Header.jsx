...
function Header() {
  ...
  return (
    <div className='header-container'>
      <div className={isActived ? 'no-show' : 'logo-container'}>
        <BsHouses />
        <Link to={'/'}><h3>UniLife</h3></Link>
      </div>
      <div className={isActived ? 'other-links other-links-actived' : 'other-links'}>
        <label className='burger' onClick={handleMenu}>{
          isActived ?
          <p>x</p>
          :
          <p>&#9776;</p>
        }</label>
        <nav className={isActived ? 'show-menu' : 'no-show'}>
